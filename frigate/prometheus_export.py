...
def export_metrics(metrics):
     collector = CustomCollector().collect(metrics.json)
...
class CustomCollector(object):
    ...
    def __init__(self, ):
                # camera stats
        self.ffmpeg_pid = GaugeMetricFamily('frigate_ffmpeg_pid', 'PID for ffmpeg process', labels=['camera'])
        self.capture_pid = GaugeMetricFamily('frigate_capture_pid', 'PID for the ffmpeg process that consumes this camera',
                                        labels=['camera'])
        self.detect_pid = GaugeMetricFamily('frigate_detect_pid', 'PID for the process that runs detection for this camera',
                                       labels=['camera'])
        self.camera_fps = GaugeMetricFamily('frigate_camera_fps', 'Frames per second being consumed from your camera.',
                                       labels=['camera'])
        self.detection_fps = GaugeMetricFamily('frigate_detection_fps', 'Number of times detection is run per second.',
                                          labels=['camera'])
        self.process_fps = GaugeMetricFamily('frigate_process_fps', 'Frames per second being processed by frigate.',
                                        labels=['camera'])
        self.skipped_fps = GaugeMetricFamily('frigate_skipped_fps', 'Frames per second skip for processing by frigate.',
                                        labels=['camera'])
        self.detection_enabled = GaugeMetricFamily('frigate_detection_enabled', 'Detection enabled for camera',
                                              labels=['camera'])
